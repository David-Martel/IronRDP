...
mod tests {
    ...
    /// Test: XCRUSH compress bells data (49 bytes).
    ///
    /// Since BELLS is only 49 bytes (≤ 50-byte threshold), XCRUSH does NOT
    /// compress it and returns the original source data with flags=0.
    #[test]
    fn test_xcrush_compress_bells() {
        ...
        // Returns flags=0 and points to the original source data.
    ...
    /// Test: XCRUSH compress island data (386 bytes).
    ///
    /// Verifies compressed output for the island test data.
    #[test]
    ...
    fn test_xcrush_roundtrip_500b_text() {
        ...
        let phrases: [&[u8]; 4] = [
            ...
            b"Crazy Frederick bought many very exquisite opal jewels. ".as_slice(),
        ];
        while data.len() < 500 {
