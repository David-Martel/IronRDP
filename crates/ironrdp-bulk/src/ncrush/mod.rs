...
impl NCrushContext {
    ...
    pub(crate) fn decompress<'a>(&'a mut self, src_data: &'a [u8], flags_value: u32) -> Result<&'a [u8], BulkError> {
        ...
        loop {
            ...
            if history_ptr >= copy_offset_usize {
                ...
                // NOTE: this continuation is folded into the
    ...
    pub(crate) fn encode_copy_offset(
        ...
        // Map raw offset to lookup index
        ...
        let lookup_idx = (lookup as usize) + 2; // +2 matches generate_tables offset
    ...
    /// Uses `(MatchLength - 2)` for the LOM table lookup but keeps
...
mod tests {
    ...
    fn test_ncrush_generate_tables_lom() {
        ...
        for i in 0..8 {
            assert_eq!(ctx.huff_table_lom[2 + i], u8::try_from(i).unwrap_or(0));
        }
    }
    ...
    fn test_ncrush_generate_tables_copy_offset() {
        ...
        for i in 0..4 {
            assert_eq!(ctx.huff_table_copy_offset[2 + i], u8::try_from(i).unwrap_or(0));
        }
    }
    ...
    fn test_ncrush_find_match_length_limit() {
        ...
        // Write identical data at two positions
        for i in 0..10 {
            ctx.history_buffer[100 + i] = u8::try_from(i).unwrap_or(0).saturating_add(1);
            ctx.history_buffer[200 + i] = u8::try_from(i).unwrap_or(0).saturating_add(1);
        }
    ...
    fn test_ncrush_move_encoder_windows_basic() {
        ...
        for i in 0..100 {
            ctx.history_buffer[32768 + i] = u8::try_from(i).unwrap_or(0).saturating_add(1);
        }
    ...
    fn test_ncrush_compress_bells() {
        ...
        assert_eq!(
            ...
            "Compressed output does not match expected bytes"
        );
    }
    ...
    /// Round-trip test with the "bells" test data.
