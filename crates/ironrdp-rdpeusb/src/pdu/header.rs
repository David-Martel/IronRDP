//! Common types shared by all PDUs defined in [MS-RDPEUSB].
//!
...
/// Unique ID for a request-response pair.
///
...
/// Indicates in what context a [`SharedMsgHeader`] is being used.
///
/// Bits 30–31 of the combined interface_id/mask word occupy the corresponding bits 0–1 of the
/// header field (big-endian meaning bits 30–31 in the u32).
///
/// * [MS-RDPEUSB § 2.2.1 SHARED_MSG_HEADER][1]
///
/// [1]: https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-rdpeusb/71cfb32c-ba15-4f95-9241-70f9df273909
...
pub enum Mask {
    /// The header is used in a response message.
    ///
    #[doc(alias = "STREAM_ID_STUB")]
    StreamIdStub = 0x2,

    /// The header is not used in a response message.
    ///
    #[doc(alias = "STREAM_ID_PROXY")]
    StreamIdProxy = 0x1,

    /// The header is used in a capabilities exchange message.
    ///
...
impl From<Mask> for u32 {
    #[expect(clippy::as_conversions, reason = "cast enum discriminant to primitive")]
...
/// Groups similar kinds of messages together, identified by a 30-bit ID.
///
/// An interface is a "group" of similar kinds of messages. Some interfaces have well-known default
/// IDs (see the associated constants), while the **USB Device** and **Request Completion**
/// interfaces receive dynamically allocated IDs during the lifetime of a USB redirection channel.
///
/// Max interface ID value: `0x3F_FF_FF_FF` (30 bits).
///
/// * [MS-RDPEUSB § 2.2.1 SHARED_MSG_HEADER][1]
///
/// [1]: https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-rdpeusb/71cfb32c-ba15-4f95-9241-70f9df273909
...
impl InterfaceId {
    /// Wire size of an encoded `InterfaceId` (combined with the [`Mask`] in the top 2 bits).
    pub const FIXED_PART_SIZE: usize = size_of::<Self>();

    /// **Exchange Capabilities** interface (ID: `0x0`).
    ///
    /// Used by both client and server to exchange capabilities for interface manipulation.
    ///
    ...
    /// **Device Sink** interface (ID: `0x1`).
    ///
    /// Used by the client to communicate with the server about new USB devices.
    ///
    ...
    /// **Channel Notification** server-to-client interface (ID: `0x2`).
    ///
    /// Used by the server to communicate with the client.
    ///
    ...
    /// **Channel Notification** client-to-server interface (ID: `0x3`).
    ///
...
impl From<u32> for InterfaceId {
    /// Constructs an `InterfaceId` from a raw u32, discarding the highest 2 bits (the mask).
...
/// Indicates the operation to perform, scoped to the interface the message belongs to.
///
/// Function IDs are defined per interface:
///
/// * Interface Manipulation Exchange Capabilities
///   * [`FunctionId::RIM_EXCHANGE_CAPABILITY_REQUEST`]
/// * Device Sink
///   * [`FunctionId::ADD_VIRTUAL_CHANNEL`], [`FunctionId::ADD_DEVICE`]
/// * Channel Notification
///   * [`FunctionId::CHANNEL_CREATED`]
/// * USB Device
///   * [`FunctionId::CANCEL_REQUEST`] … [`FunctionId::RETRACT_DEVICE`]
/// * Request Completion
///   * [`FunctionId::IOCONTROL_COMPLETION`] … [`FunctionId::URB_COMPLETION_NO_DATA`]
///
/// * [MS-RDPEUSB § 2.2.1 SHARED_MSG_HEADER][1]
///
/// [1]: https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-rdpeusb/71cfb32c-ba15-4f95-9241-70f9df273909
...
impl FunctionId {
    /// Wire size of an encoded `FunctionId`.
    pub const FIXED_PART_SIZE: usize = size_of::<Self>();

    // ── Interface Manipulation Exchange Capabilities ──────────────────────

    /// Server sends [`RimExchangeCapabilityRequest`]; client responds with
    /// [`RimExchangeCapabilityResponse`].
    ///
    /// [`RimExchangeCapabilityRequest`]: crate::pdu::caps::RimExchangeCapabilityRequest
    /// [`RimExchangeCapabilityResponse`]: crate::pdu::caps::RimExchangeCapabilityResponse
...
/// Common header (`SHARED_MSG_HEADER`) for all messages defined in [MS-RDPEUSB].
///
/// Do not use [`core::mem::size_of`] with this type — use [`SharedMsgHeader::SIZE_WHEN_NOT_RSP`]
/// or [`SharedMsgHeader::SIZE_WHEN_RSP`] depending on the direction of the message, because the
/// optional `FunctionId` field makes the encoded size variable.
///
/// * [MS-RDPEUSB § 2.2.1 SHARED_MSG_HEADER][1]
///
/// [1]: https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-rdpeusb/71cfb32c-ba15-4f95-9241-70f9df273909
...
pub struct SharedMsgHeader {
    ...
    /// Present when `mask != StreamIdStub` (i.e. the message is not a response).
    pub function_id: Option<FunctionId>,
}

impl SharedMsgHeader {
    /// Encoded size when the header is used in a response message (`mask == StreamIdStub`).
    pub const SIZE_WHEN_RSP: usize = InterfaceId::FIXED_PART_SIZE + size_of::<MessageId>();

    /// Encoded size when the header is not used in a response message.
...
impl Decode<'_> for SharedMsgHeader {
    fn decode(src: &mut ReadCursor<'_>) -> DecodeResult<Self> {
        ...
        #[expect(
            ...
            reason = "upper 2 bits fit in u8 after shift; no truncation possible"
        )]
